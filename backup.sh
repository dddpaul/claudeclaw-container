...
if [ -f "/.dockerenv" ]; then
    ...
    # Mount a backup directory into the container and point CLAUDECLAW_BACKUP_DIR
    # at it, or accept the default of /backup.
    BACKUP_DIR="${CLAUDECLAW_BACKUP_DIR:-/backup}"

    if [ ! -d "$BACKUP_DIR" ]; then
        echo "Error: backup directory '$BACKUP_DIR' does not exist inside the container."
        echo ""
        echo "Mount a host directory to the container and set CLAUDECLAW_BACKUP_DIR, e.g.:"
        echo "  docker compose run -v ~/Backups/claudeclaw:/backup claudeclaw /backup.sh"
        echo "  CLAUDECLAW_BACKUP_DIR=/backup docker compose exec claudeclaw /backup.sh"
        exit 1
    fi
